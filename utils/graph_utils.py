...
def get_truth_paths_for_strong_teacher(q_entity: list, a_entity: list, graph: nx.Graph, k = 10) -> list:
    """
        Generate the top k shortest paths for each unique pair of entities {q_entity, a_entity}
        return in the format of dict[str]: paths where key is concatenation(q, a)
    """
    # Select paths
    paths = []
    for h in q_entity:
...
def get_k_simple_paths(q_entity: list, a_entity: list, graph: nx.Graph, k=10) -> list:
    '''
    Get all simple paths connecting question and answer entities within given hop
    '''
    # Select paths
    ...
    for h in q_entity:
        ...
        for t in a_entity:
            if t not in graph:
                continue
            try:
                for p in k_shortest_paths(graph, h, t, k):
                    paths.append([(p[i], p[i + 1]) for i in range(len(p) - 1)])
            except:
                pass
    # Add relation to paths
    result_paths = []
